...
对于单（非复合）设备来说，我们使用**device_add()**（参加之前 ["Registration" section](/docs/development/drivers/developer_guide/simple.md#Registration) ）来添加设备。
...
程序首先通过声明一个`zx_device_t*`指针数组来获取设备，然后调用**device_get_fragment()**：
